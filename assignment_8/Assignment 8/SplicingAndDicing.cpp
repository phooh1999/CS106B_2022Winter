...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* headNode = new Nucleotide{str[0], nullptr, nullptr};;
    Nucleotide* prevNode = headNode;
    for (int i = 1; i < str.size(); i++) {
        Nucleotide* node = new Nucleotide{str[i], nullptr, nullptr};
        node->prev = prevNode;
        prevNode->next = node;
        prevNode = node;
    }
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    while (dna) {
        Nucleotide* check_dna = dna;
        Nucleotide* check_target = target;
        while (check_dna && check_target) {
            if (check_dna->value == check_target->value) {
                check_dna = check_dna->next;
                check_target = check_target->next;
            } else {
                break;
            }
        }
        if (check_target == nullptr) {
            return dna;
        }
        dna = dna->next;
    }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    /* TODO: Delete this comment and the next lines and implement this function. */
    Nucleotide* start = findFirst(dna, target);
    if (target == nullptr) {
        return true;
    }
    ...
    Nucleotide* preNode = start->prev;

    Nucleotide* check_dna = start;
    Nucleotide* check_target = target;
    while (check_target) {
        Nucleotide* tmp = check_dna->next;
        delete check_dna;
        check_dna = tmp;
        check_target = check_target->next;
    }
    ...
    if (preNode) {
        preNode->next = check_dna;
    } else {
        dna = check_dna;
    }

    if (check_dna) {
        check_dna->prev = preNode;
    }
