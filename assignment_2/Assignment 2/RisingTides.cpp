...
Vector<GridLocation> getAdjacent(const GridLocation &position, const Grid<double>& terrain) {
    Vector<GridLocation> allAdj;
    allAdj.add(GridLocation(position.row + 1, position.col));
    allAdj.add(GridLocation(position.row, position.col + 1));
    allAdj.add(GridLocation(position.row - 1, position.col));
    allAdj.add(GridLocation(position.row, position.col - 1));

    Vector<GridLocation> result;
    for (const GridLocation &check : allAdj) {
        if (terrain.inBounds(check)) {
            result.add(check);
        }
    }
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for (const GridLocation & water : sources) {
        if (height >= terrain.get(water)) {
            result.set(water, true);
            queue.enqueue(water);
        }
    }

    while (!queue.isEmpty()) {
        GridLocation waterPos = queue.dequeue();
        Vector<GridLocation> adjPos = getAdjacent(waterPos, terrain);
        for (const GridLocation &checkPos : adjPos) {
            if (result.get(checkPos) == false && height >= terrain.get(checkPos)) {
                result.set(checkPos, true);
