...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0) {
        error("k-gram length should be MORE than zero!!!");
    }
    ...
    int maxIndex = str.length() - kGramLength;
    for (int i = 0; i <= maxIndex; i++) {
        string checkStr = str.substr(i, kGramLength);
        if (!resultMap.containsKey(checkStr)) {
            resultMap.put(checkStr, 1);
        } else {
            resultMap[checkStr] += 1;
        }
    }
    return resultMap;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    double sumSquare = 0;
    for (const string &kGram : input) {
        sumSquare += pow(input[kGram], 2);
    }

    if (sumSquare == 0) {
        error("input Map is EMPTY or does NOT contain any nonzero values!!!");
    }

    double squareRoot = sqrt(sumSquare);
    Map<string, double> resultMap;
    for (const string &kGram : input) {
        resultMap[kGram] = input[kGram] / squareRoot;
    }
    return resultMap;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        error("numToKeep should be MORE than zero!!!");
    }
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    double sum = 0;
    for (const string &kGram : lhs) {
        if (rhs.containsKey(kGram)) {
            sum += lhs[kGram] * rhs[kGram];
        }
    }
    return sum;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    string result;
    double highSimi = 0;
    for (const Corpus &language : corpora) {
        double cosSimi = cosineSimilarityOf(textProfile, language.profile);
        if (cosSimi > highSimi) {
            highSimi = cosSimi;
