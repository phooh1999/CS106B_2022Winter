...
double weightOnBackOfRec(int row, int col, int pyramidHeight, Map<GridLocation, double> &table) {
    if (row >= pyramidHeight || col > row || col < 0 || row < 0) {
        error("position row and col should be in bound!!!");
    }
    ...
    double result = 0;
    if (col == 0) {
        result = 80 + weightOnBackOfRec(row - 1, col, pyramidHeight, table)/2;
    } else if (col == row) {
        result = 80 + weightOnBackOfRec(row - 1, col - 1, pyramidHeight, table)/2;
    } else {
        result = 160 + weightOnBackOfRec(row - 1, col - 1, pyramidHeight, table)/2
                     + weightOnBackOfRec(row - 1, col, pyramidHeight, table)/2;
    }
