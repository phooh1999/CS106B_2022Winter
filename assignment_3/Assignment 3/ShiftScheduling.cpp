 ...
 */
bool overlaps(const Shift &shift, const Set<Shift> &shifts) {
    for (const Shift &work : shifts) {
        if (overlapsWith(shift, work)) {
            return true;
        }
    }
    return false;
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if (maxHours < 0) {
        error("maxHours should be at least 0!!!");
    }
    ...
    Set<Shift> result = {};
    int maxValue = 0;
    searchScheduleRec({}, shifts, 0, 0, maxHours, maxValue, result);
    return result;
}
