 ...
 */
bool searchCity(const Map<string, Set<string>>& roadNetwork, Set<string> unsafeCities,
                int numCities, Set<string> &supplyLocations) {
    if (numCities < supplyLocations.size()) {
        return false;
    }

    if (unsafeCities.isEmpty()) {
        return true;
    }

    string checkCity = unsafeCities.first();
    // supply this city
    Set<string> coverCity = roadNetwork[checkCity] + checkCity;
    supplyLocations.add(checkCity);
    if (searchCity(roadNetwork, unsafeCities - coverCity, numCities, supplyLocations)) {
        return true;
    }
    supplyLocations.remove(checkCity);

    // supply adjacent city
    for (const string &supplyCity : roadNetwork[checkCity]) {
        if (supplyLocations.contains(supplyCity)) {
            continue;
        }
        supplyLocations.add(supplyCity);
        coverCity = roadNetwork[supplyCity] + supplyCity;
        if (searchCity(roadNetwork, unsafeCities - coverCity, numCities, supplyLocations)) {
            return true;
        }
        supplyLocations.remove(supplyCity);
    }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities < 0) {
        error("numCities should NOT be negative!!!");
    }
    Vector<string> cities = roadNetwork.keys();
    Set<string> unsafeCity = {};
    for (const string &city : cities) {
        unsafeCity.add(city);
    }
    return searchCity(roadNetwork, unsafeCity, numCities, supplyLocations);
}
