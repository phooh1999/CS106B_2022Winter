...
bool matchingRec(Map<string, Set<string>> possibleLinks, Set<string> checkNodes, Set<Pair> &matching) {
    if (possibleLinks.isEmpty()) {
        return true;
    }

    string node = possibleLinks.firstKey();
    checkNodes.add(node);
    for (const string &linkNode : possibleLinks[node]) {
        if (checkNodes.contains(linkNode)) {
            continue;
        }
        Map<string, Set<string>> used = {{node, possibleLinks[node]}, {linkNode, possibleLinks[linkNode]}};
        matching.add(Pair(node, linkNode));
        if (matchingRec(possibleLinks - used, checkNodes + linkNode, matching)) {
            return true;
        }
        matching.remove(Pair(node, linkNode));
    }
    return false;
}
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    return matchingRec(possibleLinks, {}, matching);
}

void maxWeightMatchingRec(Map<string, Map<string, int>> possibleLinks, Set<string> checkNodes,
                          Set<Pair> matching, int sumWeight, int &maxWeight, Set<Pair> &maxMatching) {
    ...
    for (const string &linkNode : possibleLinks[node]) {
        ...
        int weight = possibleLinks[node][linkNode];
        Map<string, Map<string, int>> used = {{node, possibleLinks[node]}, {linkNode, possibleLinks[linkNode]}};
        matching.add(Pair(node, linkNode));
        maxWeightMatchingRec(possibleLinks - used, checkNodes + node + linkNode, matching, sumWeight + weight, maxWeight, maxMatching);
        matching.remove(Pair(node, linkNode));
    }

    // not use node
    Map<string, Map<string, int>> used = {{node, possibleLinks[node]}};
    maxWeightMatchingRec(possibleLinks - used, checkNodes + node, matching, sumWeight, maxWeight, maxMatching);
}

Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    int maxWeight = 0;
    Set<Pair> maxMatching = {};
    maxWeightMatchingRec(possibleLinks, {}, {}, 0, maxWeight, maxMatching);
    return maxMatching;
}
