...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    m_hashFn = hashFn;
    elems = new Slot[m_hashFn.numSlots()];
    m_allocatedSize = m_hashFn.numSlots();
...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (m_logicalSize == m_allocatedSize) {
        return false;
    }
    ...
    int index = m_hashFn(elem);
    Slot insertElm{elem, 0};
    while (true) {
        if (elems[index].distance == EMPTY_SLOT) {
            elems[index] = insertElm;
            m_logicalSize++;
            return true;
        }

        if (elems[index].distance < insertElm.distance) {
            swap(elems[index], insertElm);
        }

        iterForwardIndex(index);
        insertElm.distance++;
    }
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (true) {
        ...
        int backIndex = iterBackwardIndex(index);
        elems[index].distance--;
        swap(elems[backIndex], elems[index]);
        iterForwardIndex(index);
    }
...
void RobinHoodHashTable::iterForwardIndex(int &index) const {
    index++;
...
int RobinHoodHashTable::iterBackwardIndex(int index) const {
    index--;
    if (index < 0) {
        index = m_allocatedSize - 1;
    }
    return index;
}
