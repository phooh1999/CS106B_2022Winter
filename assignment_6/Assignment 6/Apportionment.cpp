...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if (populations.size() > numSeats) {
        error("there are more states than seats!!!");
    }

    Map<string, int> result;
    int alloSeats = populations.size();
    ...
    HeapPQueue pq;
    for (const string &state : populations) {
        pq.enqueue(DataPoint(state, -populations[state] / sqrt(2)));
    }

    while (alloSeats < numSeats) {
        DataPoint state = pq.dequeue();
        result[state.name]++;
        alloSeats++;
        state.weight = -populations[state.name] / sqrt(result[state.name] * (result[state.name]+1));
        pq.enqueue(state);
    }
