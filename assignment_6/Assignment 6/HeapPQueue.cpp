...
void HeapPQueue::enqueue(const DataPoint& data) {
    /* TODO: Delete the next line and implement this. */
    if (logicalSize + 2 > allocatedSize) {
        extendArray(2*allocatedSize);
    }

    elems[logicalSize + 1] = data;
    bubbleUpEle(logicalSize + 1);
    logicalSize += 1;
}
...
void HeapPQueue::extendArray(int newSize) {
    if (newSize <= allocatedSize) {
        return;
    }

    DataPoint* newArray = new DataPoint[newSize];
...
void HeapPQueue::bubbleUpEle(int index) {
    while (index >= 2) {
        DataPoint &parent = elems[index / 2];
        DataPoint &child = elems[index];
...
void HeapPQueue::bubbleDownEle(int index) {
    while (index <= logicalSize) {
        int minIndex = index;
        if (2 * index + 1 <= logicalSize) {
            DataPoint &first  = elems[2*index];
            DataPoint &second = elems[2*index + 1];
            minIndex = first.weight <= second.weight ? 2*index : 2*index + 1;
        } else if (2 * index <= logicalSize) {
            minIndex = 2 * index;
        } else {
            return;
        }
        DataPoint &parent = elems[index];
        DataPoint &child = elems[minIndex];
        if (child.weight < parent.weight) {
            swap(child, parent);
            index = minIndex;
        } else {
