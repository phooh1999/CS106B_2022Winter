...
#include "Combine.h"
using namespace std;
void mergeVec(const Vector<DataPoint> &first, const Vector<DataPoint> &second,
                    Vector<DataPoint> &result) {
    int i = 0, j = 0;
    while (true) {
        if (i >= first.size()) {
            result += second.subList(j);
            return;
        }

        if (j >= second.size()) {
            result += first.subList(i);
            return;
        }
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> left  = combine(sequences.subList(0, half));
    Vector<DataPoint> right = combine(sequences.subList(half));

    Vector<DataPoint> result = {};
    mergeVec(left, right, result);
    return result;
}
